...
class AVH_Sup(L.LightningModule):
    def __init__(self, config):
        ...
        self.agg = self.config.get("agg", "logsumexp")
        if self.agg not in {"logsumexp", "max", "avg"}:
            raise ValueError(f"Unknown agg: {self.agg}. Choose from logsumexp|max|avg")
    ...
    def _aggregate(self, x: torch.Tensor) -> torch.Tensor:
        """
        Aggregate predictions over the temporal dimension.
        
        Args:
            x: Tensor of shape [B, T], where T corresponds to dim=-1.
        Returns:
            Tensor of shape [B].
        """
